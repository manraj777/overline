...
import { useSendOtp, useResetPassword } from '@/hooks';
...
export default function ForgotPasswordPage() {
  ...
  const handleSendOtp = async (e?: React.FormEvent) => {
    ...
    try {
      await sendOtp.mutateAsync({ phone: identifier, purpose: 'LOGIN' });
      setStep('verify');
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    setIsResetting(true);
    try {
      await resetPassword.mutateAsync({ identifier, otp, newPassword });
          ...
          <div className="flex-1 flex items-center justify-center px-8 lg:px-24">
            <div className="w-full max-w-[440px]">
              ...
              <p className="mt-8 text-center text-sm text-on-surface-variant font-medium">
                ...
                <Link href="/auth/login" className="text-primary hover:text-primary-container hover:underline underline-offset-4 font-bold transition-all">
                  Sign in
                </Link>
              </p>
            </div>
